...
export default function AnalyticsDashboard({ mode }: { mode: Mode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (mode === 'occupancy') {
        const [occupancy, tenants] = await Promise.all([analyticsAPI.getOccupancySummary(filters), analyticsAPI.getTenantsSummary(filters)]);
        setData({ occupancy: occupancy.data, tenants: tenants.data, meta: [occupancy.meta, tenants.meta] });
      } else {
        const common = await Promise.all([
          mode === 'overview' ? analyticsAPI.getOverview(filters) : analyticsAPI.getReceivablesSummary(filters),
          analyticsAPI.getCollectionsTrend(filters), analyticsAPI.getReceivablesTrend(filters),
          analyticsAPI.getStatusDistribution(filters), analyticsAPI.getRevenueByProperty(filters)
        ]);
        const [primary, collections, receivablesTrend, status, revenue] = common;
        setData({
          ...(mode === 'overview' ? { overview: primary.data as AnalyticsOverview } : { receivables: primary.data as ReceivablesSummary }),
          collections: collections.data.series,
          receivablesTrend: receivablesTrend.data.series,
          status: status.data,
          revenue: revenue.data.properties,
          meta: [primary.meta, collections.meta, receivablesTrend.meta, status.meta, revenue.meta]
        });
      }
    } catch (requestError) {
      setData(null); setError(toAnalyticsApiFailure(requestError));
  ...
  return <div className="space-y-6 text-white">
    ...
    {loading ? <AnalyticsLoading /> : error ? <AnalyticsError message={error.message} backendUnavailable={error.backendUnavailable} retry={load} /> : data ? <>
      ...
      {mode !== 'occupancy' && data.collections && data.receivablesTrend && data.status && <div className="grid gap-6 xl:grid-cols-2">
        <ChartPanel title="Recorded collection trend" subtitle="PaymentReport cash by datePaid; credit and prepaid rows excluded"><CollectionsTrendChart data={data.collections} /></ChartPanel>
        <ChartPanel title="Receivables trend" subtitle="Invoice values grouped by due date"><ReceivablesTrendChart data={data.receivablesTrend} /></ChartPanel>
        <ChartPanel title="Paid, partial and unpaid invoices" subtitle="Invoice counts; cancelled invoices excluded"><StatusDistributionChart data={data.status} /></ChartPanel>
        <ChartPanel title="Allocated collections by property" subtitle="Top properties ranked by Invoice.amountPaid"><RevenueByPropertyChart data={data.revenue || []} /></ChartPanel>
      </div>}
