...
export function CollectionsTrendChart({ data }: { data: CollectionTrendPoint[] }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ left: 4, right: 12 }}>
    <CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis dataKey="period" stroke="#94a3b8" tick={{ fontSize: 12 }} /><YAxis stroke="#94a3b8" tickFormatter={currencyTick} tick={{ fontSize: 12 }} />
    <Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Line type="monotone" dataKey="collected" name="Recorded collections" stroke="#38bdf8" strokeWidth={3} dot={{ r: 3 }} />
...
export function ReceivablesTrendChart({ data }: { data: ReceivablesTrendPoint[] }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ left: 4, right: 12 }}>
    <CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis dataKey="period" stroke="#94a3b8" tick={{ fontSize: 12 }} /><YAxis stroke="#94a3b8" tickFormatter={currencyTick} tick={{ fontSize: 12 }} />
    <Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Legend /><Bar dataKey="billed" name="Billed" fill="#38bdf8" radius={[4, 4, 0, 0]} /><Bar dataKey="paid" name="Paid" fill="#34d399" radius={[4, 4, 0, 0]} /><Bar dataKey="outstanding" name="Outstanding" fill="#fb7185" radius={[4, 4, 0, 0]} />
  </BarChart></ResponsiveContainer>;
}

export function StatusDistributionChart({ data }: { data: ReceivablesStatusDistribution }) {
  const values = [{ name: 'Paid', value: data.paid.count, color: '#34d399' }, { name: 'Partial', value: data.partial.count, color: '#fbbf24' }, { name: 'Unpaid', value: data.unpaid.count, color: '#fb7185' }];
  if (values.every(item => item.value === 0)) return <AnalyticsEmpty title="No invoice statuses" />;
  return <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={values} dataKey="value" nameKey="name" innerRadius={55} outerRadius={90} paddingAngle={3}>
    {values.map(item => <Cell key={item.name} fill={item.color} />)}
  </Pie><Tooltip contentStyle={tooltipStyle} formatter={(value) => number.format(Number(value))} /><Legend /></PieChart></ResponsiveContainer>;
}
...
export function RevenueByPropertyChart({ data }: { data: PropertyRevenue[] }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><BarChart data={data.slice(0, 10)} layout="vertical" margin={{ left: 16, right: 16 }}>
    <CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis type="number" stroke="#94a3b8" tickFormatter={currencyTick} /><YAxis type="category" dataKey="propertyName" width={105} stroke="#94a3b8" tick={{ fontSize: 11 }} />
    <Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoney(Number(value))} /><Bar dataKey="paid" name="Allocated collections" fill="#38bdf8" radius={[0, 5, 5, 0]} />
  </BarChart></ResponsiveContainer>;
}
